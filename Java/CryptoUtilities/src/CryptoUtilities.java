...
public final class CryptoUtilities {
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        NaturalNumber power = new NaturalNumber2(n);

        power.decrement();
        powerMod(wCopy, two, n);
        powerMod(w, power, n);

        if (wCopy.equals(one) || !w.equals(one)) {
            isWitness = true;
        }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            for (int i = 1; i < 50; i++) {
                NaturalNumber rnd = randomNumber(temp);
                while (rnd.compareTo(one) <= 0 || rnd.compareTo(temp) >= 0) {
                    rnd = randomNumber(temp);
                }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        // until a prime number is produced, keep adding 2 to n
        while (!isPrime2(n)) {
            n.add(two);
        }
